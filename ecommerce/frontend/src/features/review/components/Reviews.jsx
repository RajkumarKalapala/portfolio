...
export const Reviews = ({productId, averageRating}) => {
    ...
    reviews.forEach((review)=>{
        if(review.rating) ratingCounts[review.rating] = (ratingCounts[review.rating] || 0) + 1
    })
        ...
        <Stack rowGap={5} alignSelf={"flex-start"} width={is480?"90vw":is840?"25rem":'40rem'}>
                ...
                <Typography gutterBottom variant='h4' fontWeight={400}>Reviews</Typography>
                {
                    reviews?.length?(
                        ...
                        <Typography variant='h6' color={'text.secondary'} fontWeight={400}>
                            {loggedInUser?.isAdmin?"There are no reviews currently":"Be the one to post the first review"}
                        </Typography>
                    )
                }
            </Stack>

            {/* reviews list — FIX: guard against null user before accessing ._id / .name */}
            <Stack rowGap={2}>
                {reviews?.map((review)=>{
                    // Skip reviews where user was deleted or not populated
...

            {
                ...
                <Stack rowGap={3} position={'relative'} component={'form'} noValidate onSubmit={handleSubmit(handleAddReview)}>
                    <TextField id='reviewTextField' {...register("comment",{required:true})} sx={{mt:4,width:is840?"100%":"40rem"}} multiline rows={6} fullWidth placeholder='Write a review...'/>
                    <Stack>
                        <Typography gutterBottom variant='body2'>How much did you like the product?</Typography>
                        <motion.div style={{width:"fit-content"}} whileHover={{scale:1.05,x:2}} whileTap={{scale:1}}>
