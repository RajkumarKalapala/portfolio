...
// FIX: use rejectWithValue so error payload reaches action.payload in rejected cases
export const signupAsync=createAsyncThunk('auth/signupAsync',async(cred, {rejectWithValue})=>{
    try{const res=await signup(cred); return res}
    catch(err){return rejectWithValue(err)}
})

export const loginAsync=createAsyncThunk('auth/loginAsync',async(cred, {rejectWithValue})=>{
    try{const res=await login(cred); return res}
    catch(err){return rejectWithValue(err)}
})

export const verifyOtpAsync=createAsyncThunk('auth/verifyOtpAsync',async(cred, {rejectWithValue})=>{
    try{const res=await verifyOtp(cred); return res}
    catch(err){return rejectWithValue(err)}
})

export const resendOtpAsync=createAsyncThunk("auth/resendOtpAsync",async(cred, {rejectWithValue})=>{
    try{const res=await resendOtp(cred); return res}
    catch(err){return rejectWithValue(err)}
})

export const forgotPasswordAsync=createAsyncThunk('auth/forgotPasswordAsync',async(cred, {rejectWithValue})=>{
    try{const res=await forgotPassword(cred); return res}
    catch(err){return rejectWithValue(err)}
})

export const resetPasswordAsync=createAsyncThunk('auth/resetPasswordAsync',async(cred, {rejectWithValue})=>{
    try{const res=await resetPassword(cred); return res}
    catch(err){return rejectWithValue(err)}
})

export const checkAuthAsync=createAsyncThunk('auth/checkAuthAsync',async(_, {rejectWithValue})=>{
    try{const res=await checkAuth(); return res}
    catch(err){return rejectWithValue(err)}
})

export const logoutAsync=createAsyncThunk("auth/logoutAsync",async(_, {rejectWithValue})=>{
    try{const res=await logout(); return res}
    catch(err){return rejectWithValue(err)}
})
    ...
    extraReducers:(builder)=>{
        builder
            ...
            .addCase(signupAsync.rejected,(state,action)=>{
                ...
                // FIX: use action.payload (rejectWithValue) not action.error
